...
use Sys\Routing\Controller as LumenController;
...
trait RoutesRequests
{
    ...
    protected function handleNotFound(): Response
    {
        ...
        } elseif (count($views = explode('/', $view)) > 2 && !$isDynamicView) {
            $id = array_pop($views);
            $views[] = '_id';
            $view = implode('/', $views);

            if ($viewEngine->exists($view)) {
                return new Response($viewEngine->render($view, compact('id')));
            }
        }
    ...
    {
        return $this->bound('middleware.disable') && true === $this->make('middleware.disable');
    }
}
